...
from model.resnet_2d import resnet50, resnet152
...
class ConvLstm(nn.Module):
    ...
    def forward(self, x):
        ...
        conv_output, high_feat = self.conv_model(conv_input)
        lstm_input = conv_output.view(batch_size, timesteps, -1)
        lstm_output = self.Lstm(lstm_input)
        lstm_output = lstm_output[:, -1, :]
        output = self.output_layer(lstm_output)

        high_feat = high_feat.view(batch_size, timesteps, high_feat.shape[1], high_feat.shape[2], high_feat.shape[3])
...
class Pretrained_conv(nn.Module):
    def __init__(self, latent_dim):
        ...
        self.conv_model = resnet50(pretrained=True)
